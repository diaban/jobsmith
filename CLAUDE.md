...
- **The protocol runs as commands, not by hand**: `make check` (parallel, `pytest -n auto`), `make probe`, `make combo PRS="a b"` for PRs in flight, `make hooks` (ruff + `uv lock --check` at commit), `gh pr merge --auto --squash` instead of watching CI; `tests/test_decision_records.py` checks the index and every `→ NNNN`. → 0133
