...
3. If it produces a **file**: take an `ArtifactStore` (`ctx.artifacts`) in the constructor, `await self.artifacts.write(state.get("job_id", ""), "chart.svg", data)`, and pass `meta=artifact_meta(ArtifactRef(path, title="…"))` to `_emit_success`. The job lists it as an annex; never build a path yourself — `reports_dir` is the manager's.
