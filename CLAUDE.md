...
- **Shared builders live in `tests/support.py`** (`make_manager`, `make_session`, `service_over`, `make_app`, `wait_done`, `until` — wait on state, never a fixed sleep —, stub capabilities, `StubPdf`); **a test file never imports from another test file**. → 0110
